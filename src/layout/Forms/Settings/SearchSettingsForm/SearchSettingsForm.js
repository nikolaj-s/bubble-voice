...
export const SearchSettingsForm = () => {
    ...
    const { showFullResolutionPreviews, autoSendOnClick, addToMediaPlayerOnClick} = useSelector(state => state.searchSettingsSlice);

    const {disable_safe_search} = useSelector(state => state.accountPreferencesSlice);
    ...
    return (
        ...
        <ToggleSwitch initialState={disable_safe_search} onToggle={() => {dispatch(togglePreference('disable_safe_search'))}} />
        <LineSpacer />
