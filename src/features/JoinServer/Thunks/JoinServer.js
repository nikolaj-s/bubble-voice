...
export const JoinServer = createAsyncThunk(
    ...
    async ({navigate, inviteKey}, {rejectWithValue, getState, dispatch}) => {
        try {

            const {password, selectedServer} = getState().joinServerSlice;

            if (!password && !inviteKey) return rejectWithValue("Invalid Password");

            if (!selectedServer?.server_id && !inviteKey) return rejectWithValue("Invalid Server ID");
