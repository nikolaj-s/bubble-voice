...
export const ImagePreview = ({image, tags, action, tag_action, nsfw}) => {
    ...
    return (
        <div style={{backgroundColor: primaryColor}} onClick={(e) => {e.stopPropagation(); e.preventDefault(); action(image)}} href={image} className='image-preview-result-container'>
            ...
            <Image nsfw={nsfw} img_id={`image=${image}&tags=${tags}`} objectFit='cover' hideOnError={true} cursor='pointer' image={image} />
        </div>
    )
}
