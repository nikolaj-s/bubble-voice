...
export const WidgetPreview = ({widgets = [], editing = false, reorder}) => {
    ...
    return (
        <div className='widgets-preview-container'>
            ...
            : 
            <ResponsiveMasonry style={{width: 'calc(100% - 10px)', margin: '5px 5px 0px 5px'}} columnsCountBreakPoints={{1299: 1, 1300: 2, 2000: 3}}>
                <Masonry gutter='5px'>
                {widgets.map((widget, key) => {
                    return (
                        <>
                        ...
                        {widget.type === 'music' ? <MusicWidget widget={widget} key={widget._id} /> : null}
                        {widget.type === 'wheel-spin' ? <WheelSpinWidget key={widget._id} widget={widget} /> : null}
                        </>
                    )
                })}
                </Masonry>
