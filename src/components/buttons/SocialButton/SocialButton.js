...
export const SocialButton = ({action, width, padding, height, borderRadius, margin, zIndex, flip_description}) => {

    const color = useSelector(selectTextColor)

    return (
        <ButtonAnimationWrapper flip_description={flip_description} zIndex={zIndex} padding={padding} description={"Social"} margin={margin} width={width} height={height} action={action} borderRadius={borderRadius}>
