...
export const SocialButton = ({action, width, padding, height, borderRadius, margin, zIndex, flip_description, desc_o_mouse_leave, o_mouseLeave, desc_space}) => {

    const color = useSelector(selectTextColor)

    return (
        <ButtonAnimationWrapper altInvert={true} desc_space={desc_space} o_mouseLeave={o_mouseLeave} desc_o_mouse_leave={desc_o_mouse_leave} flip_description={flip_description} zIndex={zIndex} padding={padding} description={"Social"} margin={margin} width={width} height={height} action={action} borderRadius={borderRadius}>
            <svg width="18" height="18" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
<g clip-path="url(#clip0_1337_4)">
