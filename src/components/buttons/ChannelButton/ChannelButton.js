        ...
        >
            ...
            className='channel-button-container'>
                ...
                {mouseEnter ? <div className='channel-button-extra-context-wrapper'>
                    {!channel.text_only ? <SocialButton o_mouseLeave={() => {handleAnimation(transparentPrimaryColor, false)}} desc_o_mouse_leave={() => {handleAnimation(transparentPrimaryColor, false)}} flip_description={index === 0 ? true : false} zIndex={index === 0 ? 2 : 1} action={openSocial} borderRadius={4} width={20} height={20} padding={4} desc_space={12} /> : null}
                    <SubMenuButton invert={false} altInvert={true} o_mouseLeave={() => {handleAnimation(transparentPrimaryColor, false)}} desc_o_mouse_leave={() => {handleAnimation(transparentPrimaryColor, false)}} flip_description={index === 0 ? true : false} zIndex={index === 0 ? 2 : 1} description={"More"} target={`channel-button-${channel._id}`} padding={4} width={20} height={20} borderRadius={4} desc_space={12} />
                </div> : null}
            </motion.div>
            {channel.auth ?
            users.map((user) => {
